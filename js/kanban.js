  ...
  jQuery('.status').each(function (i, j) {
    ...
    if (key == undefined)
      if (i == 0) {
        ...
        jQuery('#create_new_ticket').click(function () {
          jQuery('#new_ticket').dialog({modal: true, width: '500px'});
        });
      }
      else
...
function loadTickets(pageNumber, ticketStatus) {
  var ticketUrl = '/api.php?f=tickets&s=' + ticketStatus + '&q=' + escape(currentMilestone);
  /*
...
$(document).ready(function () {
  ...
  $.when.apply($, apiPromises)
    .done(function () {
      $.each(activeMilestones, function (i, milestone) {
        $('#milestone_selector').append('<option value="' + milestone.name + '">' + milestone.name + '</option>');
      });
      $('#milestone_selector').change(function () {
        currentMilestone = $('option:selected', this).val();
